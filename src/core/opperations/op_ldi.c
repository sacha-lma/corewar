...
static int get_ldi_arg(stock_main_t *main_struct, process_t *process,
    int type, int offset)
{
...
void op_ldi(stock_main_t *main_struct, process_t *process)
{
    ...
    int arg1 = get_ldi_arg(main_struct, process, type1, 2);
    int arg2 = get_ldi_arg(main_struct, process, type2, off2);
    int reg_dest = main_struct->map[(process->pc + off3) % MEM_SIZE];
    int addr = (process->pc + (arg1 + arg2) % IDX_MOD) % MEM_SIZE;
